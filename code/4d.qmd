...
```{r}
pal <- c("cyan", "magenta", "black")
(ch <- channels(image(sd)))
```

```{r message=FALSE, results="hide"}
ax <- axes(image(sd), "name")
zi <- which(ax == "z")
nz <- dim(image(sd))[zi]-10
ps <- lapply(seq(2, nz, 2), \(z) {
    plotSpatialData() + 
        ggtitle(paste("z =", z)) +
        theme(legend.position="none") +
        plotImage(sd, z=z, t=1, ch=ch, c=pal) +
        plotLabel(sd, z=z, t=1, a=1/3, pal="yellow") +
...
ps <- lapply(seq(2, nt, 2), \(t) {
    plotSpatialData() + 
        ggtitle(paste("t =", t)) +
        theme(legend.position="none") +
        plotImage(sd, z=10, t=t, ch=ch, c=pal) +
        plotLabel(sd, z=10, t=t, a=1/3, pal="yellow") +
        scalebar(image(sd), len=10, xrel=0.9, yrel=0.9) 
})
